...
# TP Dev Avance #4 - IUT Montreuil BUT 3
#
# Usage :
#   .\load-test.ps1                          # defaut : 50 iterations
...
function Invoke-LoadTest {
    ...
    Write-Host "    Requetes: $($results.Count) | OK: $success | Erreurs: $errors" -ForegroundColor Cyan
...
try {
    $health = Invoke-WebRequest -Uri "$BaseUrl/annonces?page=0&size=1" -UseBasicParsing
    if ($health.StatusCode -eq 200) {
        Write-Host "  [OK] Serveur accessible (200)" -ForegroundColor Green
    }
}
catch {
    Write-Host "  [ERR] Le serveur ne repond pas ($($_.Exception.Response.StatusCode)). Verifiez que Tomcat tourne." -ForegroundColor Red
    exit 1
}
Write-Host ""
...
try {
    $loginObj = @{ username = "john"; password = "password" }
    ...
    Write-Host "  [ERR] Login echoue. Verifiez JAAS et la BDD." -ForegroundColor Red
    exit 1
}
Write-Host ""
...
Write-Host "[Phase 3] Lancement des $Iterations iterations..." -ForegroundColor Yellow
Write-Host ""
...
$createObj = @{ title = "Load test PS"; description = "Annonce test de charge PowerShell"; price = 99.99; category = "Electronique" }
...
# Test 4 : POST /login (auth repetee)
$loginBodyStr = (@{username = "john"; password = "password" } | ConvertTo-Json -Compress)
Invoke-LoadTest -Label "POST_login" -Method "POST" -Url "$BaseUrl/auth/login" -Body $loginBodyStr

# --- Rapport -----------------------------------------------------------------

Write-Host "===================================================" -ForegroundColor Green
Write-Host "Resultats detailles dans : $ResultsFile" -ForegroundColor Green
